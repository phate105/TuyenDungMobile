...
export default function EmployerApplicationsScreen({ navigation, user }) {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function loadData() {
        try {
          setLoading(true);
          const rows = await employerService.getApplicationsByEmployer(user.id, { status: activeFilter, limit: 1000, offset: 0 });
          if (active) {
            setItems(rows);
          }
        } catch (error) {
    ...
    return (
      <View style={styles.header}>
        ...
        <Text style={styles.countText}>
          {filteredItems.length} kết quả • Trang {page}/{totalPages}
        </Text>
      </View>
    );
  }
  ...
  function renderItem({ item }) {
    const initial = item.candidate_name?.charAt(0)?.toUpperCase() || "A";
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardInfo}>
            <Text style={styles.cardTitle} numberOfLines={1}>
              {item.candidate_name || "Ứng viên"}
            </Text>
            <Text style={styles.cardCompany} numberOfLines={1}>
              {item.job_title || "Chưa rõ vị trí"}
            </Text>

            <View style={styles.metaRow}>
              <StatusBadge status={item.status} />
              <MetaItem icon="call-outline" value={item.candidate_phone || "Chưa cập nhật ĐT"} />
            </View>
          </View>
